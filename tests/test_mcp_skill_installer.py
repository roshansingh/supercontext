...
class McpSkillInstallerTest(unittest.TestCase):
    def test_project_install_copies_only_supercontext_mcp_skills(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("source-inspection head start", codex_skill)
            self.assertIn("single behavior contract", codex_skill)
            self.assertIn("call `planning_context` first", codex_skill)
            self.assertIn("call `review_context` first", codex_skill)
            self.assertIn("ordinary source inspection", codex_skill)
            self.assertIn("exact primitive tools", codex_skill)
            self.assertNotIn("answerability.status", codex_skill)
            self.assertNotIn("operational_surfaces", codex_skill)
            ...
            self.assertIn("source-inspection head start", claude_skill)
            self.assertIn("single behavior contract", claude_skill)
            self.assertIn("call `planning_context` first", claude_skill)
            self.assertIn("call `review_context` first", claude_skill)
            self.assertIn("ordinary source inspection", claude_skill)
            self.assertIn("exact primitive tools", claude_skill)
            self.assertNotIn("answerability.status", claude_skill)
            self.assertNotIn("operational_surfaces", claude_skill)
