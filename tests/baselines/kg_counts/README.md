...
- `true_loop` JS/TS endpoint caller resolution v1: `Endpoint` 19 -> 18, `CALLS_ENDPOINT` 26 -> 25, and coverage 6 -> 13 because parser-backed client-call extraction now suppresses one external full-URL call (`https://api.vapi.ai/call/{}`) instead of treating it as an internal endpoint, and emits per-call-site coverage for one `external_endpoint_suppressed` and six `unresolved_target` fetch/axios targets.
