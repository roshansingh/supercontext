...
`latticeai_23.json` is captured from the public `data/kg_runs/latticeai_23` snapshot. The canonical validation report uses `data/kg_runs/private_goldset_eval_2026_05_11`, which currently applies the remaining private Zappa example extension on top of public OSS extraction. Apache/WSGI vhost extraction is public OSS again. Small count differences between these two artifacts are expected; compare each artifact only against the snapshot it names.
