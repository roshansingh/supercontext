...
- JS/TS route parser widening: no `EXPOSES_ENDPOINT` count change in the current public snapshots; coverage reason changed from `parser_backed_js_ts_route_extraction_partial_express_only` to `parser_backed_js_ts_route_extraction_partial_express_fastify_koa_only` because the parser now supports literal Express, Fastify, and Koa route receivers. Historical local snapshots may carry the old reason and are normalized by the baseline-capture tool during comparison.
