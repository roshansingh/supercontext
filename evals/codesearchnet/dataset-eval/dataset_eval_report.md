...
**Corpus:** 457,461 Python functions (all splits)  
**Queries:** 92 evaluated (99 total, 7 skipped due to <3 URL matches)  
**Runtime:** 211s  
**Source paper:** Husain et al., "CodeSearchNet Challenge: Evaluating the State of Semantic Code Search" (2019)

---

## Leaderboard — NDCG Within (Python)

NDCG computed only over human-annotated functions. Measures how well the approach 
re-ranks the ~5-10 annotated candidates relative to each other.
...
### Why our "Within" scores are so high

Our TF-IDF indexes **docstrings** (`func_documentation_string`) — the same text 
that the CodeSearchNet queries were derived from. This is essentially keyword matching 
on documentation. The published neural baselines operate on **code tokens only** 
(docstrings stripped at evaluation time), which makes the task much harder.

Our high "Within" scores demonstrate that **docstring-based retrieval is extremely 
effective for keyword queries**, but this metric alone doesn't tell the full story.

---

## Leaderboard — NDCG All (Python)

NDCG computed over ALL 457K functions (top-1000 ranking window). This is the harder, 
more meaningful metric — can you find the right function among 457K candidates?
...
On "All", keyword matching alone isn't enough — too many functions contain similar 
keywords. ElasticSearch (BM25 with proper length normalization and field boosting) 
outperforms our simple TF-IDF. However:

- **SC-enhanced beats TF-IDF baseline** on this metric (31 wins vs 10 wins, 51 ties)
- The structural boost (matching query terms to function calls, imports, domain keywords 
  in the code) adds marginal but consistent value when discriminating among many similar 
  functions
- We beat biRNN (0.064) and approach 1D-CNN (0.166)

---

## SuperContext's Real Value: Not Semantic Search

These NDCG scores measure **semantic code search** (NL query → code snippet). This is 
**not** SuperContext's primary use case. SuperContext is a **structural knowledge graph** 
for change-safety queries:

| Capability | Semantic search (this eval) | SuperContext's strength |
|------------|---------------------------|----------------------|
| Query type | "convert json to csv" | "what calls this function?" |
| Retrieval unit | Single function | Call graph, dependency edges, blast radius |
| Evidence | Ranked snippets | Commit-pinned, typed, transitive |
| Tool calls | 1 per query | 1 per query (vs 7 for an LLM agent) |

See the **Three-Way Eval** (grep vs Claude Code vs SuperContext) for the structural 
comparison where SuperContext wins 15/16 queries.
...
On the harder "All" metric, the structural boost from code features (function calls, 
imports, domain alignment) gives SC-enhanced a 3:1 win ratio over plain TF-IDF.

---

## Methodology

1. **Corpus:** Full `code_search_net/python` from Hugging Face (457,461 functions across all splits)
2. **Annotations:** 2,079 human relevance judgments from `annotationStore.csv` (0-3 scale) across 99 queries
3. **TF-IDF baseline:** Index `func_documentation_string` + `func_name`, score with TF-IDF
4. **SC-enhanced:** TF-IDF + structural boost from:
   - Domain keyword alignment (ML, web, data, crypto, IO, math, string, date, collection)
   - Function call name matching via AST parse
   - Import module matching via AST parse
5. **NDCG Within:** Sort annotated functions by their rank in the full-corpus ranking, compute NDCG
6. **NDCG All:** Compute NDCG over the top-1000 functions in the full-corpus ranking
