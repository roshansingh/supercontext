...
- `source/kg/product/mcp_skill_templates/{codex,claude}/supercontext-mcp/SKILL.md` are the installable Codex and Claude Code skill templates for planning, coding, and review flows.
- `source/scripts/install_mcp_skills.py` installs only those explicit `supercontext-mcp` host skill templates into project-local or global Codex/Claude Code skill directories. It intentionally does not copy this repository's project-maintenance skills.
...
- `install.sh` is the one-line machine installer. It installs the SuperContext package, global host-agent MCP skills, and default host MCP registration.
- `supercontext-init` is the per-repo setup command. It builds the repo-local KG snapshot at `.supercontext/kg` by default, and `supercontext-init --serve` builds that snapshot and starts the local MCP server in the foreground.
...
- Build KG snapshots locally per repository because facts, paths, commit evidence, coverage, and freshness are repo-specific. The default project artifact is `.supercontext/kg`.
...
- For writing code, SuperContext should not replace reading source files. It should reduce blind search by answering exact graph questions such as who calls a symbol, what a symbol calls, which event channel has producers or consumers, and which service or dependency anchor is relevant.
...
- Skill support now has an installer for explicit global or project-local `supercontext-mcp` skill files, and `supercontext-register-mcp` can register the default local MCP endpoint with Codex and Claude Code. Host behavior still needs real-world evaluation: we need to measure whether installed skills and registered MCP tools reliably cause agents to call SuperContext at planning, coding, and review time.
