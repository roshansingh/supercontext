...
The v0 KG can extract Apache virtual-host mappings when a config block has both a domain directive and a WSGI entrypoint. Current behavior is intentionally strict:
...
Facts emitted by deterministic config extractors should be canonical v0 facts with `derivation_class = deterministic_static`, subject to ADR-0006 promotion and evidence-envelope rules.
...
- Adds one more predicate to the v0 fact vocabulary.
- Query and synthesis layers must merge route facts and deploy-only facts carefully.
- The v0 `DeployTarget` shape still needs future alignment with ADR-0006's canonical deployment ontology.
...
- Apache adapter capability metadata to claim `DEPLOYS_VIA_CONFIG`;
...
- ADR-0006 still owns the canonical ontology and fact envelope. This ADR defines a v0 deploy-target evidence relation for local KG evaluation; future ontology work may map `DeployTarget` / `DEPLOYS_VIA_CONFIG` into canonical `Deployable` / `Deployment` terms.
- ADR-0004 still owns canonical-vs-candidate separation. Deterministic config facts covered by this ADR are canonical v0 facts; candidate deploy facts must remain in explicit candidate/enrichment paths.
- ADR-0009 reverse dependency semantics are unchanged; deploy topology is not a reverse dependency query.

## References

- `source/kg/extraction/config/apache_vhost.py`
- `source/kg/extraction/adapters/config_apache_vhost.py`
