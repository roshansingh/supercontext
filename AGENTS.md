...
- Apply **Head-Start Budgeting** at the MCP transport boundary, not by weakening internal semantic APIs. Internal query/tool functions can return exact structured packets for tests and local composition; the agent-facing MCP response should be grep-shaped rows (`locator [tag] category fact`) plus `gaps` and `next`.
- Account for MCP wire duplication when choosing caps: the current transport includes the compact packet in both `content[].text` and `structuredContent`, so one-copy caps must leave room for the doubled host message.
- Prefer complete relevant rows for small packets and prioritized source-pointer rows for large packets. A good bounded transport response surfaces the highest-value review/planning leads first, with evidence coordinates and why each lead matters.
...
- Use Head-Start Budgeting only when full detail does not fit: detailed grep-style rows for the most important facts, plus `gaps`/`next` guidance for omitted or unproven areas. Rows should include the relevant fact/evidence summary; follow-up guidance should still include enough coordinates or search terms for the agent to inspect without rediscovering the row from scratch.
...
- If rows must be truncated, expose an explicit continuation/narrowing contract: `more` plus the exact narrower anchor or source search in `next`. Never let truncation silently erase a category.
...
- Preserve candidate/unlinked leads as candidate-tagged rows, such as `[candidate:unlinked_source_lead]`, instead of hiding them inside omitted counts or merging them into `[proven]` rows.
- Track packet-navigation tax after eval runs. If `mcp_packet_saved_file_count` or `mcp_packet_jq_attempt_count` is nonzero, inspect whether a tool response escaped the inline budget and caused packet-file archaeology instead of useful source inspection.
