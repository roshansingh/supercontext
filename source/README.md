...
The canonical validation report is generated by `supercontext-run-validation` (`source.scripts.run_product_validation`). Public contributors can run the deterministic checks that have local fixtures available; maintainers with private snapshots can additionally include private goldset packets, answers, and judgements. Older dated evaluation artifacts should stay out of the repo unless a sanitized run is promoted as canonical.
...
.venv/bin/python -m unittest tests.test_baseline_drift tests.test_product_query_matrix_drift
.venv/bin/python -m source.scripts.compare_snapshot_baseline data/kg_runs/<snapshot_dir> --baseline tests/baselines/kg_counts/<corpus_name>.json
.venv/bin/supercontext-run-validation --query-matrix-md-out docs/evaluation/PRODUCT-QUERY-SET-RUN.md
```
...
For upgrades from BetterContext, remove stale `bettercontext` MCP registrations and old `~/.bettercontext` scripts after verifying the new `supercontext-*` commands work.
...
The examples below use `.venv/bin/` so they work from a source checkout even when console scripts are not on PATH. Installed users can omit that prefix after PATH setup.

```bash
.venv/bin/supercontext-build-kg --repo /path/to/service-repo --out data/kg_runs/service_repo
.venv/bin/supercontext-build-multi-kg --repo /path/to/frontend --repo /path/to/backend --out data/kg_runs/multi_repo_fixture
.venv/bin/supercontext-build-multi-kg --repo /path/to/frontend --repo /path/to/backend --out data/kg_runs/multi_repo_fixture --strict-extractors

.venv/bin/supercontext-query-kg --snapshot data/kg_runs/service_repo summary
.venv/bin/supercontext-query-kg --snapshot data/kg_runs/service_repo modules-importing requests --limit 5
.venv/bin/supercontext-query-kg --snapshot data/kg_runs/service_repo dependency-info os
.venv/bin/supercontext-query-kg --snapshot data/kg_runs/service_repo top-dependencies --limit 10
.venv/bin/supercontext-query-kg --snapshot data/kg_runs/service_repo find-callers handle_request --limit 5
.venv/bin/supercontext-query-kg --snapshot data/kg_runs/service_repo lookup-symbol handle_request

.venv/bin/supercontext-query-kg --snapshot data/kg_runs/multi_repo_fixture cross-repo-links --limit 10
.venv/bin/supercontext-query-kg --snapshot data/kg_runs/multi_repo_fixture repo-dependencies backend --limit 10
.venv/bin/supercontext-query-kg --snapshot data/kg_runs/multi_repo_fixture domain-references api.example.com --limit 20
.venv/bin/supercontext-query-kg --snapshot data/kg_runs/multi_repo_fixture endpoints --path /api/token --limit 20
.venv/bin/supercontext-query-kg --snapshot data/kg_runs/multi_repo_fixture event-channels --channel orders-events --limit 20
.venv/bin/supercontext-query-kg --snapshot data/kg_runs/multi_repo_fixture deploy-mappings --target app_wsgi.py --limit 20
.venv/bin/supercontext-query-kg --snapshot data/kg_runs/multi_repo_fixture reconcile-contract --name docs_vs_backend --identity-key endpoint_path --left-name documented --left-predicate DOCUMENTS_ENDPOINT --left-repo api_docs --left-path-prefix /v1/ --right-name implemented --right-predicate EXPOSES_ENDPOINT --right-repo backend --right-path-prefix /v1/
```

Maintainer-only product validation, when the required fixture snapshots and private goldset files are present:

```bash
.venv/bin/python examples/private-goldset/run_scenario.py --snapshot data/kg_runs/private_goldset --scenario <scenario-id>
.venv/bin/python examples/private-goldset/run_scenario.py --snapshot data/kg_runs/private_goldset --scenario <scenario-id> --out data/kg_runs/private_goldset/product_packets.json
.venv/bin/supercontext-run-validation \
  --mercury-snapshot data/kg_runs/<python-smoke-snapshot> \
  --true-loop-snapshot data/kg_runs/<typescript-smoke-snapshot> \
  --private-snapshot data/kg_runs/private_goldset \
  --goldset-packets data/kg_runs/private_goldset/product_packets.json \
  --goldset-answers data/kg_runs/private_goldset/goldset_answers.json \
  --goldset-judgement data/kg_runs/private_goldset/goldset_judgement.json
```
