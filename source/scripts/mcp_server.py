...
Every tool result includes `packet_contract`, `answerability`, `proven_facts`, `covered_areas`, `candidate_leads`, `coverage_gaps`, and `inspection_areas`. Some large planning packets also include `output_budget`, and multi-intent packets may include `intent_answerability`. Treat these as the normalized first-read fields across all tools. Use `covered_areas` and `proven_facts` to identify already-covered KG-backed/static sections, then cite the underlying evidence rows or file/line coordinates. Use `intent_answerability` to decide whether the packet answers the user's specific intent; a packet can answer endpoint inventory while still being partial for deploy order, schema safety, runtime usage, ownership, or authz completeness. Use `candidate_leads` and `inspection_areas` as the bounded source-inspection plan for uncovered tests, scripts, notebooks, entry points, import-only consumers, config, manifests, runtime routes, or other areas outside the proven packet. If `output_budget.truncated` is true, use omitted counts and backfilled counts as truncation metadata, not absence proof; follow returned inspection refs/search terms or call narrower anchors for omitted categories that matter. Use `coverage_gaps` to state what the graph could not prove. Do not claim candidate leads, missing gaps, or unsupported scopes as facts until source inspection verifies them.

## Answerability

Read answerability, intent_answerability, covered_areas, proven_facts, candidate_leads, coverage_gaps, inspection_areas, coverage_warnings, unsupported_scopes, and next_actions before finalizing. Do not rediscover already-covered proven facts unless verification risk requires it; spend source inspection on inspection_areas, candidate leads, omitted rows, runtime gaps, ambiguous anchors, and user-requested categories not covered by the packet. If a SuperContext result is partial, ambiguous, unsupported_by_current_kg, or not_found, say what the graph could not prove and inspect the relevant workspace source files with ordinary Read/Grep before finalizing. For runtime event time windows and deploy-safety claims, static graph facts are context only; inspect operational/config/source evidence. Do not treat a graph miss as proof of absence.
...
For PR review impact, read review_context.review_answer_packet first, then review_context.application_impact alongside framework_impact and runtime_surfaces. When the prompt names expected impact categories such as UI screens, scheduled jobs, SQS consumers, delivery workers, tracking paths, API surfaces, models, serializers, or schemas/contracts, pass requested_surfaces so surface_status can separate known, unlinked, and missing evidence. application_impact groups same-repo app/package namespace surfaces such as API, models, serializers, workers, and scheduled jobs, and separates app-scoped runtime facts from unlinked cross-repo name leads. Use cross_repo_name_leads only as source-inspection leads, not as proven impact.
"""
