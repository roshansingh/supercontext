...
Read answerability, proven_facts, candidate_leads, coverage_gaps, inspection_areas, coverage_warnings, unsupported_scopes, and next_actions before finalizing. If a SuperContext result is partial, ambiguous, unsupported_by_current_kg, not_found, or not_answerable, say what the graph could not prove and use your normal search/read tools once before finalizing. For runtime event time windows and deploy-safety claims, static graph facts are context only; inspect operational/config/source evidence. Do not treat a graph miss as proof of absence.
