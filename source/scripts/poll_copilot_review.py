...
def _poll_once(repo: str, pr_number: int, head_sha: str, head_created_at: str) -> dict[str, Any]:
    ...
    inline_comments = _gh_json(["api", f"repos/{repo}/pulls/{pr_number}/comments"])
    ...
    copilot_inline = [
        ...
        if _is_copilot_user(comment.get("user")) and comment.get("commit_id") == head_sha
    ]
    copilot_issue = [
        comment
    ...
    return {
        "activity": bool(copilot_events or copilot_reviews or copilot_inline or copilot_issue),
        "feedback": bool(copilot_reviews or copilot_inline or copilot_issue),
        "reviews": copilot_reviews,
        "inline_comments": copilot_inline,
...
def _print_summary(result: dict[str, Any], attempt: int, repo: str, pr_number: int, head_sha: str) -> None:
    ...
    for comment in result["inline_comments"]:
        print(f"- inline: {comment.get('path')} {comment.get('html_url')}")
