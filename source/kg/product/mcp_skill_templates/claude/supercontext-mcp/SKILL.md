...
For every MCP result, read `packet_contract`, `answerability`, `intent_answerability`, `proven_facts`, `covered_areas`, `candidate_leads`, `coverage_gaps`, and `inspection_areas` before deciding what to inspect next. `covered_areas` and `proven_facts` point to KG-backed/static fields and their counts; cite the underlying detailed rows or file/line evidence, not just the index. `intent_answerability` separates what the packet can answer for the user's specific intent from what remains partial. `candidate_leads` contains plausible but unverified leads such as import-only consumers, unlinked runtime evidence, ambiguous candidates, or inferred guidance. `coverage_gaps` lists what the KG could not prove. If `output_budget` is present, treat it as truncation guidance and use `inspection_areas`, `output_budget.omitted_counts`, `output_budget.backfilled_counts`, and narrower anchors to recover omitted detail when the task needs it.

Use SuperContext as a head start for source inspection: do not reread every proven row when `covered_areas` shows the packet already covers it, unless verification risk requires it. Spend source inspection on `inspection_areas`, candidate leads, omitted rows, runtime gaps, ambiguous anchors, and user-requested categories not covered by the packet. If the packet is compacted or too large, prioritize the most task-relevant `inspection_areas`; still name the other concrete refs/search terms as follow-up areas when they matter. Count-only omissions are not enough for final claims; follow any provided refs/search terms or call a narrower anchor when omitted categories are relevant. Do not claim candidate leads, missing gaps, or unsupported scopes as facts until source inspection verifies them.
...
Read `summary`, `inventory`, `entry_points`, `related_facts`, `source_coordinates`, `intent_answerability`, and the common packet fields before deciding what to inspect next.
