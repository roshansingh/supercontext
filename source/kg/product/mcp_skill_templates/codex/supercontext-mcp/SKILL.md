...
Read `summary`, `entry_points`, `related_facts`, `source_coordinates`, and `answerability` before deciding what to inspect next. Use `source_coordinates` for targeted source reads instead of starting with broad grep.
...
- `get_service_brief` for a concise service fact sheet when no broader planning or impact context is needed.
