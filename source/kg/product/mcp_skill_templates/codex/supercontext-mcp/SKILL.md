...
The primary `limit` controls top-level result rows. Nested packets such as `entry_points`, `related_facts`, and `source_coordinates` are intentionally capped by the returned `summary.section_limit` to keep planning context compact. Fleet planning packets use a compact output cap; anchored planning packets allow more detail but are still bounded. For runtime architecture questions, read `runtime_architecture.answer_packet`; if `output_budget` is present and `truncated` is true, use `output_budget.omitted_counts` and call `planning_context` again with narrower `repo`, `service`, `domain`, or `endpoint` anchors for omitted detail.
