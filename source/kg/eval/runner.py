...
async def async_run_single_task(
    ...
    *,
    ...
    # Claude can emit a successful final ResultMessage while leaving SDK background-task markers incomplete.
    # Keep the answer usable for A/B comparison, but record/tag the IDs so trace analysis can flag the host anomaly.
...
def _raise_for_host_error_messages(
    ...
    *,
    fail_on_incomplete_background_tasks: bool = True,
) -> None:
    """Raise for failed/missing host results; set flag false only for recorded A/B ingestion."""
