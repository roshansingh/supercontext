...
| Q089 | Hard | `blast_radius` / CLI | Growth engineer | `mercury_api`, `mercury_campaign_messages`, `mercury_tracking` | If campaign message payloads change, which schedulers, delivery workers, and tracking services are affected? | Cross-service path from campaign producer to delivery consumer to tracking, with unknown producer gaps explicit. | Async workflow impact, multi-hop traversal. |
| Q090 | Hard | Support / CLI | Marketing ops | `mercury_campaign_messages`, `mercury_tracking`, `mercury_api` | Where are email opens, clicks, bounces, and delivery events handled across the system? | Event handlers and tracking domains grouped by event type, source system, and repo. | Event taxonomy, tracking-domain linkage. |
| Q091 | Hard | `get_event_producers` / CLI | Platform | `mercury_webhooks`, `mercury_api`, `shopagain_api_docs` | Which external webhook sources enter the platform, and where do their events get normalized or processed? | Webhook domains, source systems, handler symbols, downstream processors, and docs references. | Webhook ingestion map, source-to-handler linking. |
| Q092 | Hard | Support / CLI | Support engineer | `shopagain-chat-widget`, `mercury_websocket`, `mercury_api`, `mercury_ui`, `ShopAgainMobile` | What repos participate in live chat, from customer widget to websocket to backend API and operator UI? | End-to-end chat path with routes, env vars, services, and UI callers. | Real-time feature topology, frontend/backend traversal. |
| Q093 | Medium | `blast_radius` / CLI | Engineer | `mercury_websocket`, `shopagain-chat-widget`, `ShopAgainMobile` | If websocket route `postChatMessage` changes, which clients and backend callbacks are affected? | Route handler, client senders/listeners if indexed, API callback URL, and missing extraction caveats. | WebSocket route impact, serverless handler lookup. |
...
| Q095 | Medium | `blast_radius` / CLI | SRE | `ansible-playbooks`, `mercury_api`, frontend clients | If the `prod_shopagain_wsgi.py` deployment changes, which public domains and clients are impacted? | Domain-to-WSGI mapping, client baseURLs pointing to that domain, and backend repo evidence. | Reverse deploy lookup, client impact. |
| Q096 | Hard | Support / CLI | SRE | `mercury_api`, `mercury_campaign_messages` | Which scheduled jobs can send customer messages or trigger campaign work? | Zappa scheduled events, callable handlers, queue targets, and cadence metadata. | Scheduled job extraction, async side-effect audit. |
| Q097 | Medium | Support / CLI | SRE | `mercury_ui`, `ShopAgainMobile`, `shopagain-chat-widget`, backends | Where are Sentry, Amplitude, Firebase, or similar observability clients initialized? | Observability libraries and initialization files grouped by repo and runtime. | Observability coverage query, dependency-to-symbol lookup. |
| Q098 | Hard | Support / CLI | Security | `latticeai` org | Which repos touch customer PII such as email, phone, customer metadata, or billing identifiers, and which vendors may receive it? | PII touchpoints, external vendors, file citations, and confidence/source class for each edge. | Security data-flow discovery, vendor dependency graph. |
| Q099 | Medium | `blast_radius` / CLI | Backend engineer | `hipo-drf-exceptions`, `mercury_api`, client repos | If `hipo-drf-exceptions` changes API error response shape, which services and clients should be validated? | Package dependency edge, backend usage sites, frontend/mobile exception consumers if present, and test recommendations. | Shared error contract impact, package linking. |
| Q100 | Hard | Support / CLI | API owner | `shopagain_api_docs`, `mercury_api`, client repos | Which documented ShopAgain API endpoints are not obviously implemented or called by any client? | Endpoint diff table: documented, implemented, called, unknown, with citations and false-positive caveats. | Docs-code drift detection, endpoint reconciliation. |
| Q101 | Hard | Support / CLI | API owner | `mercury_ui`, `ShopAgainMobile`, `shopagain-chat-widget`, `shopagain_api_docs` | Which client API calls are missing from public API docs? | Client endpoint inventory minus docs inventory, grouped by repo and risk. | Reverse docs coverage, frontend API inventory. |
| Q102 | Medium | Support / CLI | Platform | `latticeai` org | Which staging and production domains are configured across repos, and are any clients pointing to the wrong environment? | Env var/domain table by repo with staging/prod classification and suspicious mismatches. | Environment config audit, domain normalization. |
| Q103 | Hard | `blast_radius` / CLI | Product engineer | `mercury_api`, `mercury_ui`, `shopagain_api_docs` | If Shopify OAuth or store-install flow changes, which UI steps, backend handlers, and docs need updates? | Install/auth endpoints, UI route/actions, backend symbols, docs references, and external Shopify edge. | Third-party integration impact analysis. |
...
| Q108 | Medium | Support / CLI | Platform | `latticeai` org | Which languages and repo types in this fixture are currently under-indexed for reliable cross-repo answers? | Coverage table by repo/language/file type with blocked query IDs and next extractor suggestions. | Coverage reporting, extractor gap prioritization. |
| Q109 | Hard | Support / CLI | Marketing ops | `mercury_api`, `mercury_tracking`, `ansible-playbooks` | Which services implement or depend on shortlink and tracking domains like `ltai.io` and `shopagainmail.net`? | Domain mappings, WSGI/apps, URL handlers if indexed, and client/campaign references. | Domain-to-feature linkage, tracking impact. |
| Q110 | Hard | Support / PR bot | Tech lead | `mercury_api`, `mercury_ui`, `mercury_campaign_messages`, `mercury_tracking` | A PR changes a campaign model in `mercury_api`. Which UI screens, scheduled jobs, SQS consumers, delivery workers, and tracking paths are affected? | Multi-hop blast radius with direct and transitive edges, citations, confidence tier, and pagination/depth metadata. | End-to-end feature blast radius, graph traversal quality. |
